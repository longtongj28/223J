...
public class BouncingPanel extends JPanel {
    ...
    protected void paintComponent(Graphics g) {
        ...
        if (initialize) {
            ...
            int half_height = frameHeight/2;
            int half_width = frameWidth/2;
            g2.drawLine(0, half_height , frameWidth, half_height);
            g2.drawLine(half_width, 0, half_width, frameHeight);
            int width_tracker = 0;
            int height_tracker = 0;
            String width_as_string = Integer.toString(width_tracker - half_width);
            String height_as_string = Integer.toString(height_tracker - half_height);
            while (width_tracker < frameWidth) {
                g2.drawString(width_as_string, width_tracker, half_height);
                width_tracker+=100;
                width_as_string = Integer.toString(width_tracker - half_width);
            }
            while (height_tracker < frameHeight) {
                g2.drawString(height_as_string, frameWidth/2, height_tracker);
                height_tracker+=100;
                height_as_string = Integer.toString(height_tracker - half_height);
            }
            // g2.
    ...
    public void reset(int height, int width, double catPosX, double catPosY ) {
        ...
        // point1x = width / 2;
        // point1y = height / 2;
        ball_center_x = (double) 0;
        ball_center_y = (double) Math.sin(0);
